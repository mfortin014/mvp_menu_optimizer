...
SELECT
    NULL::uuid AS recipe_line_id,
    NULL::uuid AS recipe_id,
    NULL::uuid AS ingredient_id,
    NULL::numeric AS qty,
    NULL::text AS qty_uom,
    NULL::numeric AS package_qty,
    NULL::text AS package_uom,
    NULL::numeric AS package_cost,
    NULL::text AS ingredient_type,
    NULL::numeric AS yield_pct,
...
CREATE VIEW public.prep_costs AS
 SELECT r.id AS recipe_id,
    ...
    (r.yield_qty * c.factor) AS yield_qty_in_base_unit,
        CASE
            WHEN ((r.yield_qty * c.factor) > (0)::numeric) THEN (sum(rlc.line_cost) / (r.yield_qty * c.factor))
            ELSE NULL::numeric
        END AS unit_cost,
    c.to_uom AS base_uom
   FROM ((public.recipes r
     LEFT JOIN public.recipe_line_costs rlc ON ((r.id = rlc.recipe_id)))
     LEFT JOIN public.ref_uom_conversion c ON ((r.yield_uom = c.from_uom)))
  WHERE ((r.recipe_type = 'prep'::text) AND (r.status = 'Active'::text))
  GROUP BY r.id, r.recipe_code, r.name, r.yield_qty, r.yield_uom, c.factor, c.to_uom;
...
CREATE VIEW public.recipe_summary AS
 SELECT r.id AS recipe_id,
    ...
    sum(rlc.line_cost) AS total_cost,
        CASE
            WHEN (r.price > (0)::numeric) THEN round(((sum(rlc.line_cost) / r.price) * (100)::numeric), 2)
            ELSE NULL::numeric
        END AS cost_pct,
        CASE
            WHEN (r.price > (0)::numeric) THEN round((r.price - sum(rlc.line_cost)), 2)
            ELSE NULL::numeric
        END AS margin
   FROM (public.recipes r
     LEFT JOIN public.recipe_line_costs rlc ON ((r.id = rlc.recipe_id)))
...
--
-- Name: recipe_line_costs _RETURN; Type: RULE; Schema: public; Owner: -
--
...
CREATE OR REPLACE VIEW public.recipe_line_costs AS
 SELECT rl.id AS recipe_line_id,
   ...
   FROM (((public.recipe_lines rl
     ...
     LEFT JOIN public.ref_uom_conversion c ON (((rl.qty_uom = c.from_uom) AND (i.package_uom = c.to_uom))))
     LEFT JOIN public.prep_costs pc ON ((pc.recipe_id = rl.ingredient_id)));
