...
if "package_cost" in display_df:
    display_df["package_cost"] = display_df["package_cost"].apply(
...
with st.sidebar:
    ...
    with st.form("ingredient_form"):
        ...
        category_id = (
            next((c["id"] for c in categories if c["name"] == picked_cat_name), None)
            if picked_cat_name != "— Select —"
            else None
        )
        ...
        storage_type_id = (
            next((s["id"] for s in storage_types if s["name"] == picked_storage), None)
            if picked_storage != "— Select —"
            else None
        )
